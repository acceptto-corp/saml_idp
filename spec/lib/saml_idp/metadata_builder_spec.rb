...
module SamlIdp
  describe MetadataBuilder do
    ...
    it "includes logout element" do
      subject.config.single_logout_service_post_location = 'https://example.com/saml/logout'
    ...
    context "technical contact" do
      before do
        subject.config.technical_contact.company       = nil
        subject.config.technical_contact.given_name    = nil
        subject.config.technical_contact.sur_name      = nil
        subject.config.technical_contact.telephone     = nil
        subject.config.technical_contact.email_address = nil
      end

      it "all fields" do
        subject.config.technical_contact.company       = "ACME Corporation"
        subject.config.technical_contact.given_name    = "Road"
        subject.config.technical_contact.sur_name      = "Runner"
        subject.config.technical_contact.telephone     = "1-800-555-5555"
        subject.config.technical_contact.email_address = "acme@example.com"
    ...
    it "includes logout element as HTTP Redirect" do
      subject.config.single_logout_service_redirect_location = 'https://example.com/saml/logout'
